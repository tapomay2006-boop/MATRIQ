...
export default function NationalSidebar({ onCloseMobile }: NationalSidebarProps) {
  ...
  const navGroups = [
    {
      group: "OVERVIEW",
      items: [
        { name: "Dashboard / Reports", href: "/dashboard_national", icon: LayoutDashboard },
      ],
    },
    {
      group: "MATERIAL DATA",
      items: [
        { name: "All Materials (Whole DB)", href: "/materials", icon: Package },
      ],
    },
    {
      group: "HARMONIZATION",
      items: [
        { name: "Harmonization Review", href: "/reviews", icon: CheckSquare },
      ],
    },
  ];

  const userName = session?.user?.name || "National Director";
  const userEmail = session?.user?.email || "director@ministry.gov.in";
  const userInitial = (session?.user?.name?.trim() || session?.user?.email?.trim() || "N").charAt(0).toUpperCase();
    ...
    <aside className="w-64 sm:w-68 h-full bg-[#0E0C0F] border-r border-white/10 flex flex-col justify-between p-4 shrink-0 select-none z-30">
      ...
      <div className="flex flex-col gap-6 overflow-y-auto no-scrollbar pr-1">
        ...
        <div className="flex items-center justify-between pt-2 px-1">
          <Link href="/dashboard_national" className="flex items-center gap-2.5 no-underline group">
            <MatriqLogo size={32} />
            <span
            ...
            <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-bold bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 uppercase tracking-wider ml-1">
              NATIONAL
            </span>
          </Link>
        ...
        <div className="px-3 py-2 rounded-xl bg-white/[0.03] border border-white/10 flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <span className="text-[11px] font-semibold text-white">National Admin Workspace</span>
            <span className="text-[9px] text-zinc-400">Central Directory · Sovereign</span>
          </div>
        </div>
          ...
          {navGroups.map((group) => (
            <div key={group.group} className="flex flex-col gap-1">
              ...
              {group.items.map((item) => {
                const isActive =
                  pathname === item.href ||
                  (item.href !== "/dashboard_national" && pathname.startsWith(item.href));
      ...
      <div className="pt-3 border-t border-white/10 flex flex-col gap-2">
        <div className="flex items-center justify-between p-2 rounded-xl bg-white/[0.02] border border-white/5">
          ...
          <button
            ...
            title="Sign out of National authority account"
          >
