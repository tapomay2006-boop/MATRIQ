"""Phase 1: raw ERP text -> the eight canonical CPSE attributes.

    VENDORED FROM backend/pipeline-one/inference_engine.py

This file is Rakesh's pipeline-one extraction engine, brought into ai-service
so the service that owns the vector embedding DB also owns the code that
produces what goes into it. It is a copy, not a fork: behaviour here must stay
identical to pipeline-one's, and the only local edits are the adapter path and
the abbreviation-table path, both of which now resolve inside ai-service.

    Re-sync rule: if pipeline-one's inference_engine.py changes, copy it over
    this file again and re-apply the two path edits marked [AI-SERVICE].

What it does, and what it deliberately does not:

  * A fine-tuned Qwen2.5-3B (`qwen2.5-3b-cpse-lora-v2`) reads one un-delimited
    catalogue string and emits JSON keyed by the eight canonical attributes.
  * `ground_attributes` then throws away anything the model did not actually
    read in the input, which is what keeps a generative model out of the
    business of inventing part numbers.
  * `expand_abbreviations` applies the CPSE domain taxonomy.

  * It does NOT classify, score, compare or match. Everything downstream of
    the standard format in ai-service is embedding and the vector DB.
"""
...
# [AI-SERVICE] Additional definitions from this service's own config directory.
# In pipeline-one this reached sideways into ../ai-service/data/config; here it
# is simply local, which is the point of vendoring the engine.
_abbrev_csv_path = (
    Path(__file__).resolve().parent.parent.parent
    / "data" / "config" / "abbreviations.csv"
)
...
class QwenLoraEngine:
    ...
    def __init__(
        ...
        # [AI-SERVICE] Locate the adapter. The caller passes it in
        # (services/extraction.py resolves settings.lora_adapter_dir); the
        # fallback is this service's own data/models directory.
        if adapter_dir:
            self.adapter_dir = adapter_dir
        else:
            root = Path(__file__).resolve().parent.parent.parent
            self.adapter_dir = str(root / "data" / "models" / "qwen2.5-3b-cpse-lora-v2")
